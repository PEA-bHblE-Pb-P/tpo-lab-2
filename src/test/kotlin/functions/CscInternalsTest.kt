package functions

import functions.system.Csc
import org.assertj.core.data.Offset
import org.junit.jupiter.api.DynamicTest
import org.junit.jupiter.api.TestFactory
import util.Round.roundStLibValue
import util.assertByOffset
import kotlin.math.sin

class CscInternalsTest {
    private val csc = Csc(sin = { x, _ -> sin(x).roundStLibValue() })
    private val eps = 0.00001
    private val offset = Offset.offset(eps)

    @TestFactory
    fun `csc pi times(n) should be equal to NaN`(): Collection<DynamicTest> {
        val range = (-5..5).toList()

        return range.map { i ->
            DynamicTest.dynamicTest("csc(PI*$i) when sin(PI*$i) is ${sin(Math.PI * i)}") {
                assertByOffset(csc(Math.PI * i, eps), Double.NaN, offset)
            }
        }
    }
}